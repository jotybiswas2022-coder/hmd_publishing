    ...
    private function syncIndexed(string $prefix, string $type, array $fieldMap, ?string $starsField = null): void
    {
        ...
        foreach ($titles as $i => $title) {
            ...
            $data = [
                'key'          => $prefix . '_' . ($i + 1),
                'section_type' => $type,
                'title'        => $fieldMap['title'] ? request()->input($fieldMap['title'])[$i] ?? null : null,
                'description'  => $fieldMap['description'] ? request()->input($fieldMap['description'])[$i] ?? null : null,
                'content'      => $fieldMap['content'] ? request()->input($fieldMap['content'])[$i] ?? null : null,
                'icon'         => $fieldMap['icon'] ?? null ? request()->input($fieldMap['icon'])[$i] ?? null : null,
                'url'          => $fieldMap['url'] ?? null ? request()->input($fieldMap['url'])[$i] ?? null : null,
                'meta'         => $meta,
                'is_active'    => $isActive,
                'sort_order'   => $i,
            ];
